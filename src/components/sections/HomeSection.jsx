...
export default function HomeSection({ content, statistics }) {
    // 기본값 설정 (fallback용)
    const defaultContent = {
        title: "Error",
        subtitle: "Error",
        description: "Error",
        primary_button: "Error",
        secondary_button: "Error"
    };
                    ...
                    <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-3">
                        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                            <div className="text-3xl font-bold text-yellow-400">
                                {statsContent?.creators_value || '150+'}
                            </div>
                            <div className="text-sm text-gray-300 mt-1">
                                {statsContent?.creators_label || '등록된 크리에이터'}
                            </div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                            <div className="text-3xl font-bold text-pink-400">
                                {statsContent?.projects_value || '80+'}
                            </div>
                            <div className="text-sm text-gray-300 mt-1">
                                {statsContent?.projects_label || '성공한 협업 프로젝트'}
                            </div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                            <div className="text-3xl font-bold text-blue-400">
                                {statsContent?.companies_value || '45+'}
                            </div>
                            <div className="text-sm text-gray-300 mt-1">
                                {statsContent?.companies_label || '파트너 게임 회사'}
                            </div>
                        </div>
                    </div>
