...
export default function AdminPanel({ onDataUpdate }: AdminPanelProps) {
    ...
    const handleClearCache = () => {
        ...
        setLastSync(null);
        setSyncStatus(null);
        alert('캐시가 삭제되었습니다. 다음 페이지 로드 시 새로운 데이터를 가져옵니다.');
    };
    ...
    return (
        <div className="fixed bottom-4 right-4 bg-white border border-gray-200 rounded-lg shadow-xl p-6 w-80 z-50">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-900">관리자 패널</h3>
                <button
            ...
            <div className="space-y-4">
                ...
                <div className="text-xs text-gray-500 border-t pt-2">
                    <div>API 키: {process.env.NEXT_PUBLIC_GOOGLE_SHEETS_API_KEY ? '✅ 설정됨' : '❌ 미설정'}</div>
                    <div>시트 ID: {process.env.NEXT_PUBLIC_GOOGLE_SHEETS_SPREADSHEET_ID ? '✅ 설정됨' : '❌ 미설정'}</div>
                </div>
            </div>
        </div>
    );
